                        ...
                        int radius, int width, OUTLINE_MODE tracemode, void* aData )
{
    ...
    else
    {
        ...
        Arc( centre, StAngle, EndAngle,
             radius - ( width - m_currentPenWidth ) / 2, FILL_T::NO_FILL, -1 );
        Arc( centre, StAngle, EndAngle,
             radius + ( width - m_currentPenWidth ) / 2, FILL_T::NO_FILL, -1 );
    }
}
...
void PLOTTER::FilledCircle( const wxPoint& pos, int diameter, OUTLINE_MODE tracemode, void* aData )
{
    if( tracemode == FILLED )
    {
        Circle( pos, diameter, FILL_T::FILLED_SHAPE, 0 );
    }
    else
    {
        SetCurrentLineWidth( -1 );
        Circle( pos, diameter, FILL_T::NO_FILL, -1 );
    }
}


void PLOTTER::PlotPoly( const SHAPE_LINE_CHAIN& aCornerList, FILL_T aFill, int aWidth,
                        void* aData )
{
