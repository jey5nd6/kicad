...
void SCH_PAINTER::draw( const LIB_SHAPE *aShape, int aLayer )
{
    ...
    if( setDeviceColors( aShape, aLayer ) )
    {
        ...
        case SHAPE_T::ARC:
        {
            m_gal->DrawArc( mapCoords( aShape->GetPosition() ), aShape->GetRadius(),
                            DECIDEG2RAD( aShape->GetArcAngleStart() ),
                            DECIDEG2RAD( aShape->GetArcAngleEnd() ) );
        }
            break;
        ...
        case SHAPE_T::POLYGON:
        {
            ...
            for( const VECTOR2I& pt : aShape->GetPolyShape().Outline( 0 ).CPoints() )
