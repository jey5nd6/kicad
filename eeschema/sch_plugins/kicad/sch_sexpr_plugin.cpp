                                                 ...
                                                 int aNestLevel )
{
    ...
    case LIB_SHAPE_T:
    {
        ...
        case SHAPE_T::CURVE:   saveBezier( shape, aFormatter, aNestLevel );    break;
        case SHAPE_T::POLYGON: savePolyLine( shape, aFormatter, aNestLevel );  break;
        default: ;
        }
...
{
    aFormatter.Print( aNestLevel,
                      "(arc (start %s %s) (mid %s %s) (end %s %s))",
                      FormatInternalUnits( aArc->GetStart().x ).c_str(),
                      FormatInternalUnits( aArc->GetStart().y ).c_str(),
                      FormatInternalUnits( aArc->GetArcMid().x ).c_str(),
                      FormatInternalUnits( aArc->GetArcMid().y ).c_str(),
                                         ...
                                         int aNestLevel )
{
    ...
    for( const wxPoint& pt : { aBezier->GetStart(),
                               aBezier->GetBezControl1(),
                               aBezier->GetBezControl2(),
                               aBezier->GetEnd() } )
    {
