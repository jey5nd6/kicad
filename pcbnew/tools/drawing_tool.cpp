...
int DRAWING_TOOL::DrawArc( const TOOL_EVENT& aEvent )
{
    ...
    while( drawArc( tool, &arc, immediateMode ) )
    {
        ...
        arc = m_isFootprintEditor ? new FP_SHAPE( parentFootprint, SHAPE_T::ARC )
                                  : new PCB_SHAPE( nullptr, SHAPE_T::ARC );
        arc->SetFlags( IS_NEW );
        immediateMode = false;
    }
...
void DRAWING_TOOL::constrainDimension( PCB_DIMENSION_BASE* aDim )
{
    ...
    aDim->Update();
#if 0
    else if( aStep == SET_TEXT )
    {
        if( aDim->Type() == PCB_DIM_RADIAL_T )
        {
            PCB_DIM_RADIAL* radialDim = static_cast<PCB_DIM_RADIAL*>( aDim );
            VECTOR2I        lineVector{ radialDim->Text().GetPosition() - radialDim->GetKnee() };
            VECTOR2I        snapped = GetVectorSnapped45( lineVector );

            radialDim->Text().SetPosition( radialDim->GetKnee() + (wxPoint) snapped );
        }
        else if( aDim->Type() == PCB_DIM_LEADER_T )
        {
            VECTOR2I lineVector{ aDim->Text().GetPosition() - aDim->GetEnd() };
            VECTOR2I snapped = GetVectorSnapped45( lineVector );

            aDim->Text().SetPosition( aDim->GetEnd() + (wxPoint) snapped );
        }
    }
#endif
}
        ...
        else if( evt->IsMotion() )
        {
            ...
            case SET_END:
                ...
                else if( dimension->Type() == PCB_DIM_RADIAL_T )
                {
                    ...
                    wxPoint         textOffset( radialDim->GetArrowLength() * 5, 0 );
                ...
                {
                    wxPoint textOffset( dimension->GetArrowLength() * 5, 0 );
                                ...
                                OPT<VECTOR2D> aStartingPoint )
{
    ...
    while( TOOL_EVENT* evt = Wait() )
    {
        ...
        {
            // 45 degree lines
            if( started && ( ( limit45 && shape == SHAPE_T::SEGMENT )
