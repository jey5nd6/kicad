                                                   ...
                                                   const VECTOR2I aCursor ) const
{
    ...
    else
    {
        ...
        if( arcValid )
            aArc->UpdateArcGeometry( aStart, aEnd, aCenter );
    }
}
                                                  ...
                                                  const VECTOR2I aCursor ) const
{
    ...
    VECTOR2I *p1, *p2;
    bool     minAngle = aArc->GetArcAngle() < 1800;
...
bool PCB_POINT_EDITOR::canAddCorner( const EDA_ITEM& aItem )
{
    ...
    {
        const PCB_SHAPE& shape = static_cast<const PCB_SHAPE&>( aItem );
        return shape.GetShape() == SHAPE_T::SEGMENT
               || shape.GetShape() == SHAPE_T::POLYGON;
    }
...
bool PCB_POINT_EDITOR::removeCornerCondition( const SELECTION& )
{
    ...
    case PCB_SHAPE_T:
    case PCB_FP_SHAPE_T:
        if( static_cast<PCB_SHAPE*>( item )->GetShape() == SHAPE_T::POLYGON )
...
int PCB_POINT_EDITOR::addCorner( const TOOL_EVENT& aEvent )
{
    ...
    if( item->Type() == PCB_ZONE_T || item->Type() == PCB_FP_ZONE_T
        || ( graphicItem && graphicItem->GetShape() == SHAPE_T::POLYGON ) )
    {
...
int PCB_POINT_EDITOR::removeCorner( const TOOL_EVENT& aEvent )
{
    ...
    else if( item->Type() == PCB_FP_SHAPE_T || item->Type() == PCB_SHAPE_T )
    {
        ...
        if( shape->GetShape() == SHAPE_T::POLYGON )
